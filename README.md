...
一個 Windows 常駐語音輸入工具。連按兩下錄音鍵開始錄音，再按一次停止；程式會使用 Groq Whisper 轉寫，整理繁體中文、半形英文標點、AI／科技品牌拼法，然後貼到目前的輸入位置。預設錄音鍵是 Menu，也可以從右下角通知區圖示自行更換。
...
5. 將游標放在想輸入文字的位置，連按兩下預設的 Menu 鍵開始錄音，再按一次停止；辨識完成後會自動貼上文字。

通知區的 AZA-STT 圖示可隨時確認程式仍在執行。按兩下圖示可查看狀態；
按右鍵可設定錄音按鍵、重新輸入 Groq API key、開啟 Groq API Keys 網頁，
或完整退出程式。錄音按鍵設定會儲存在本機，變更後立即生效。
